...
class RuntimeNode:
    def __init__(self, node_model: nodes.Node) -> None:
        ...
        self.flow_next = node_model.flow.output_target
    ...
    def initialize(self, all_nodes: dict[int, "RuntimeNode"]) -> None:
        ...
        for raw_output in self.node_model.outputs:
            ...
            self.out_sources[raw_output.name] = RuntimeSourcePtr(target_id, rt_input_node)
        
        if self.flow_next is not None:
            self.flow_next = all_nodes.get(self.flow_next.node_hash)
