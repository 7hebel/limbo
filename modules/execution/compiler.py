...
class Compiler:
    ...
    def __init__(self, name: str, start_node: Node, all_nodes: list[Node]) -> None:
        ...
        build_target_content += self.include_dependencies() + "\n"
        build_target_content += defined_fns + "\n"
        build_target_content += self.build_instances_register() + "\n"
        build_target_content += "print('hello world')"
    ...
    def include_dependencies(self) -> str:
        ...
        print(f"Included {style.highlight(str(len(self.__dependecies_imports)))} dependencies.")
        return content
    ...
    def build_instances_register(self) -> str:
        ...
        Register format:
        {
            'node_id': {
                ...
                '__next-flow': targetNodeID / None,                   (by default set to flow-output, 
        ...
        for node in self.all_nodes:
            ...
            if node.flow.enable_output and node.flow.output_src.target is not None:
                ...
                node_data['flow-output'] = flow_target
                node_data['__next-flow'] = flow_target
    ...
    def compile_to_exe(self) -> str:
        ...
        shutil.move(f"{self.__compilation_dir_path}{output_exe}", f"{self.__compilation_dir_path}../")
